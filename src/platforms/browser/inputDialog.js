...
var css = {
  ...
  getStylesheet: function () {
    ...
    return new dom.Stylesheet('.' + this.classes.base).add('',
      'position: absolute; width: 100%; height: 100%; top: 0px; left: 0px; z-index: 9999999; background: rgba(0, 0, 0, 0.5);'
    ).add('*', prefix + 'box-sizing: border-box; font: 12px Helvetica').add(
      '.' + this.classes.dialog,
      'position: absolute; color: #444; background: rgba(255, 255, 255, 0.93); border-radius: 5px; text-align: left; ' +
      prefix + 'box-shadow: 0px 3px 40px rgba(0, 0, 0, 0.2)').add('.' +
      this.classes.hasMessage, 'margin-bottom: 16px').add('.' + this.classes
      .title,
      'padding: 16px; font-weight: bold; overflow: hidden; text-overflow: ellipsis; white-space: nowrap'
    ).add('.' + this.classes.message, 'padding: 0px 16px; color: #888').add(
      '.' + this.classes.buttons,
      'margin: 16px 0px 0px 0px; height: 34px; border-top: 1px solid #CCC'
    ).add('.' + this.classes.input, 'padding: 0px 8px').add('button',
      'position: relative; border: 0px; margin: 0px; width: 50%; height: 100%; cursor: pointer; overflow: hidden; white-space: nowrap; text-overflow: ellipsis; font-weight: bold; background: transparent; outline: none'
    ).add('button:active', 'background: #FFF').add('button:first-child',
      'border-bottom-left-radius: 5px').add('button:last-child',
      'border-bottom-right-radius: 5px').add('button:only-child',
      'width: 100%; border-bottom-right-radius: 5px').add(
      'button:nth-child(2):before',
      'content:""; position: absolute; left: 0px; top: 0px; width: 1px; height: 100%; border-left: 1px solid #CCC'
    ).add('input',
      'margin: 0px; border: 1px solid #ccc; padding: 4px 8px; text-align: left; width: 100%'
    );
  },
  sizeAndPositionDialog: function (dialog, contents) {
    ...
    contents.style.maxHeight = Math.min(200 * scale, Math.floor(windowSize.height /
      3)) + 'px';
    dialog.style.maxWidth = Math.min(300 * scale, Math.floor(windowSize.width -
      40)) + 'px';
    dialog.style.left = Math.max(0, (windowSize.width - dialog.offsetWidth) /
      2) + 'px';
    dialog.style.top = Math.max(0, (windowSize.height - dialog.offsetHeight) /
      2) + 'px';
  }
};
