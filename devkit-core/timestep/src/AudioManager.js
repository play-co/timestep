...
class MultiSound {
  ...
  constructor (soundManager, name, opts) {
    ...
    // Loading priority from 0 (no priority) to 10 Hight priority
    var priority = opts.priority !== undefined ? opts.priority : 0;
    ...
    for (var i = 0; i < srcList.length ; ++i) {
      ...
      /// #if BUILD_TYPE == 'wechat'
      fullPath = window.DATA_HOST + fullPath;
      /// #endif
      ...
      if (!this._useAudioContext) {
        ...
        if (audio) {
          ...
          /// #if BUILD_TYPE == 'wechat'
          this._delaySoundLoading(priority, audio, fullPath);
          /// #else
          audio.src = fullPath;
          /// #endif
  ...
  _delaySoundLoading (priority, audio, src) {
    let timeout = priority * 400 + (100 * Math.random());
    if (priority === 0) { timeout = 0; }
    setTimeout(() => { audio.src = src; }, timeout);
  }
  ...
  getDuration () {
    if (this._useAudioContext) {
      return this.lastSrc && this.lastSrc.buffer && this.lastSrc.buffer.duration || 0;
    }
  ...
  set lastSrc (src) {
    if (this.isBackgroundMusic && this._lastSrc && this._lastSrc !== src) {
...
export default class AudioManager extends Emitter {
  ...
  constructor (opts) {
    ...
    var soundName;
    for (soundName in this._map) {
      this.addSound(soundName, this._map[soundName]);
    }

    // AudioContext preloading
    if (_ctx && this._preload) {
      for (soundName in this._sounds) {
        loader._loadSound(this._sounds[soundName]._paths);
      }
    }
  }
...
AudioManager.prototype.playBackgroundMusic = AudioManager.prototype.play;
// expose a global mute function (GC.app.muteAll)
AudioManager.muteAll = function (mute) {
  _muteAll = mute;
