...
export default class InputDialog {
  constructor (controller, opts) {
    ...
    if (addClasses) {
      css.sizeAndPositionDialog(this.dialog, body);
    }
  ...
  _createButton (text, cb) {
    ...
    $.onEvent(btn, 'mouseup', () => this.onInputEnd(cb));
    $.onEvent(btn, 'touchend', () => this.onInputEnd(cb));
...
InputDialog.show = function (controller, opts) {
