...
export default class Input {
  ...
  constructor (opts) {
    ...
    document.addEventListener('touchmove', this.handleMouse.bind(this, eventTypes.MOVE), { passive: false });
    document.addEventListener('mousemove', this.handleMouse.bind(this, eventTypes.MOVE), { passive: false });
    document.addEventListener('mouseup', this.handleMouse.bind(this, eventTypes.SELECT));
    document.addEventListener('touchend', this.handleMouse.bind(this, eventTypes.SELECT));
    window.addEventListener('DOMMouseScroll', this.handleMouse.bind(this, eventTypes.SCROLL));

    // FF
    window.addEventListener('mousewheel', this.handleMouse.bind(this, eventTypes.SCROLL), { passive: false });
  ...
  _addElEvents () {
    ...
    // @TODO @FIXME `document.addEventListener` returns `undefined`.
    // We should fix the unbinding in `_removeElEvents()` method
    this._elEvents.push(document.addEventListener('mousedown', this.handleMouse.bind(this, eventTypes.START)));
    this._elEvents.push(document.addEventListener('touchstart', this.handleMouse.bind(this, eventTypes.START)));
  ...
  handleMouse (type, evt) {
    ...
    if (evt.touches) {
      ...
      for (var i = 0; i < changedTouches.length; i++) {
        this.handleMouse(type, changedTouches[i]);
      }
      return;
    }
