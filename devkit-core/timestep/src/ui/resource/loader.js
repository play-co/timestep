...
import { logger, CONFIG } from 'base';
...
class Loader {
  ...
  _updateImageMap (map, url, x, y, w, h) {
    ...
    /// #if PLATFORM == 'wc'
    // Extract path from url in wechat builds. Leave as is in other builds.
    if (/^http/.test(url)) {
  ...
  _loadAsset (url, loadMethod, cb, priority, isExplicit, index) {
    ...
    if (!/^(http|https:)/.test(url)) {
      url = window.DATA_HOST + url;
    }
    /// #endif
    if (!loadMethod) {
      logger.warn('Invalid load method for: ' + url);
      return cb && cb(null, index);
    }

    if (!url) {
      logger.warn('The image url is empty.');
      return cb && cb(null, index);
    }
  ...
  _getImageURL (id) {
    var path = id;
    if (/^https?:\/\/[^\/]+\//.test(path)) {
      var parts = /^https?:\/\/[^\/]+\/(.*)$/.exec(path);
      path = parts[1] || '/';
    }
    var spritesheet = this._map[path];
    if (spritesheet) {
  ...
  constructURLs (pathPrefix) {
    var urls;
    if (typeof pathPrefix !== 'string' && pathPrefix instanceof Array) {
      urls = [];
