...
var imageSizeOverrides = {};

exports.overrideImageSize = function (url, width, height) {
  // TODO: resize cached Images
  // Difficult to do for now: needs unified Image type in timestep

  imageSizeOverrides[url] = { width: width, height: height };
};
...
function _loadImage (url, cb, loader) {
  var img = new Image();
  ...
  img.onload = function () {
    ...
    var image = this;
    var sizeOverride = imageSizeOverrides[url];
    if (sizeOverride) {
      var canvas = document.createElement('canvas');
      var width = sizeOverride.width;
      var height = sizeOverride.height;
      canvas.width = width;
    ...
    var asset = webglSupported ? new TextureWrapper(image) : image;
    return onRequestComplete(cb, asset);
  };
...
function _loadFile (url, cb, loader, priority, isExplicit, responseType) {
  ...
  // TODO: add loading options for credentials
  xobj.withCredentials = loader._password ? true : false;
  xobj.open('GET', url, true, loader._user, loader._password);
  xobj.send();
}
...
exports.loadSound = function (url, cb, loader, priority, isExplicit) {
  loadFile(url, sound => {
    ...
    // In case our browser don't support HTML 5 audio context,
    // we will call callback without proper decoded sound buffer
    // See issue EW-1876
    if (!audioContext) {
      return cb && cb(null);
    }
