...
var textFlowMode = enumeration('NONE', 'WRAP', 'AUTOSIZE', 'AUTOSIZE_WRAP',
...
export default class TextFlow extends PubSub {
  ...
  _lineSplit (ctx) {
    ...
    if ((opts.hardWrap || TextFlow.hardWrap) && words.length && this.measureText(
      ...
      while (i < text.length) {
        var char = text[i++];
        if (ctx.measureText(s + char).width >= availableWidth) {
          words.push(s);
          s = char;
        } else {
          s += char;
    ...
    while (currentWord < wordCount) {
      ...
      this._line.push(wordData);
      this._lineWidth += wordData.width + spaceWidth;
      this._maxWordWidth = Math.max(this._maxWordWidth, wordData.width);
      currentWord++;
    }
  ...
  reflow (ctx, mode) {
    ...
    switch (mode) {
      ...
      case textFlowMode.AUTOFONTSIZE_WRAP:
        ...
        var lineCount = this._lines.length;
        lineSize = this._getLineSize(lineCount);
    ...
    if (words.length) {
      ...
      lineSize = Math.ceil(opts.size * opts.lineHeight + strokeWidth);
