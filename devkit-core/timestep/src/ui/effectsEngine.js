...
var parameterPool, propertyPool, particlePool, effectPool;
...
class Particle {
  constructor () {
    ...
    for (var j = 0; j < OTHER_KEY_COUNT; j++) {
      var otherKey = OTHER_KEYS[j];
      this[otherKey] = OTHER_DEFAULTS[otherKey];
    }
  ...
  reset (effect) {
    ...
    for (var i = 0; i < OTHER_KEY_COUNT; i++) {
      var otherKey = OTHER_KEYS[i];
      var defaultValue = OTHER_DEFAULTS[otherKey];
      if (typeof defaultValue === 'number') {
        // support value, range, and params for ttl and delay
        this[otherKey] = getNumericValueFromData(effect, data[otherKey], defaultValue);
      } else {
        this[otherKey] = data[otherKey] || defaultValue;
      }
    }

    // reset animated properties with data or to their defaults
    for (var j = 0; j < PROPERTY_KEY_COUNT; j++) {
    ...
    for (var k = 0; k < POLAR_KEY_COUNT; k++) {
      if (this[POLAR_KEYS[k]].isModified) {
        this.isPolar = true;
        break;
      }
    }
    ...
    for (var l = 0; l < STYLE_KEY_COUNT; l++) {
      var styleKey = STYLE_KEYS[l];
      s[styleKey] = this[styleKey].value;
    }
    s.flipX = this.flipX;
    s.flipY = this.flipY;
  ...
  step (dt) {
    ...
    if (this.hasDeltas) {
      for (var i = 0; i < STYLE_KEY_COUNT; i++) {
        var styleKey = STYLE_KEYS[i];
        var styleProperty = this[styleKey];
        if (styleProperty.delta) {
          styleProperty.applyDelta(dt);
          s[styleKey] = styleProperty.value;
        }
      }

      if (this.isPolar) {
        for (var j = 0; j < POLAR_KEY_COUNT; j++) {
          var polarKey = POLAR_KEYS[j];
          var polarProperty = this[polarKey];
          polarProperty.delta && polarProperty.applyDelta(dt);
        }
      }
    }
...
parameterPool = new ObjectPool({ ctor: Parameter });
propertyPool = new ObjectPool({ ctor: Property });
particlePool = new ObjectPool({ ctor: Particle });
effectPool = new ObjectPool({ ctor: Effect });
...
function validateEffect (data) {
  ...
  if (params) {
    ...
    for (var j = 0; j < params.length; j++) {
      validateParameter.call(this, params[j], paramList, data);
    }
  }
 ...
 */
export default class EffectsEngine extends View {
  constructor () {
    super({
    ...
    setTimeout(bind(this, function () {
      engine.on('Tick', bind(this, onTick));
    }), 0);
  }
